...
- (void)applicationDidFinishLaunching:(UIApplication *)application {
	...
	[prayerNavController setTabBarItem:[[UITabBarItem alloc] initWithTitle:@"Prayers" image:prayersTabImg tag:0]];
...
- (void)initializeDatabase {
	NSString *xmlPath = [NSBundle pathForResource:@"BPiP" ofType:@"xml" inDirectory:[[NSBundle mainBundle] resourcePath]];
	NSFileHandle *xmlFile = [NSFileHandle fileHandleForReadingAtPath:xmlPath];
	NSXMLParser *parser = [[NSXMLParser alloc] initWithData:[xmlFile availableData]];
	[parser setDelegate:[PrayerDatabase sharedInstance]];
	[parser parse];
